    ...
    return (
      <div className="container mx-auto px-4 py-16 min-h-screen">
        <Card className="p-8 max-w-2xl mx-auto">
          ...
          <p className="text-gray-400 text-center mb-6">{error.message || 'Error desconocido'}</p>
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">🏆 Colecciones</h1>
            <p className="text-gray-400">Completa sets para desbloquear bonuses de minería</p>
          </div>
          {bonusSummary && (
        ...
        {bonusSummary && bonusSummary.totalBonus > 0 && (
          <Card variant="glass" className="p-6 mb-6 border-2 border-green-500/50 bg-green-500/10">
        ...
        {allSets?.map((set) => {
          ...
          const totalRequired = set.requiredCounts.reduce((sum, count) => sum + count, 0);
          const totalOwned = progress?.requirements.reduce((sum, req) => sum + req.owned, 0) ?? 0;
          const progressPercent = totalRequired > 0 ? (totalOwned / totalRequired) * 100 : 0;
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-white font-bold">{totalRequired} NFTs</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <TrendingUp className="w-4 h-4 text-green-400" />
                      <span className="text-gray-400">Bonus:</span>
                      <span className="text-green-400 font-bold">+{set.bonusPercentage}%</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {progress.requirements.map((req, idx) => (
                      <div key={idx} className="bg-gray-800 rounded p-3">
                        <div className="text-xs text-gray-400 mb-1">{req.typeName}</div>
        ...
        <h2 className="text-xl font-bold text-white mb-4">💡 Sobre el Sistema de Colecciones</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-300">
          <div>
            <h3 className="font-bold text-white mb-2">¿Cómo funcionan los Sets?</h3>
            <p>
              Los sets son colecciones temáticas de NFTs. Al completar un set (poseer todos 
              los NFTs requeridos), obtienes un bonus permanente de minería que se acumula 
              con otros sets completados.
            </p>
          </div>
          <div>
        ...
        <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
          ...
          <p className="text-sm text-gray-300">
            Los NFTs en sets no necesitan estar stakeados para recibir el bonus. 
            Puedes mantenerlos en tu wallet o stakearlos para obtener beneficios adicionales.
          </p>
        </div>
        </Card>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link href="/forge">
            ...
            <h3 className="text-lg font-bold text-white mb-2">Forjar Geodas</h3>
          ...
          <Link href="/inventory">
            ...
            <div className="text-4xl mb-3">📦</div>
            <h3 className="text-lg font-bold text-white mb-2">Ver Inventario</h3>
            <p className="text-sm text-gray-400">
