...
export default function TrustScorePage() {
    ...
    return (
      <div className="container mx-auto px-4 py-16 min-h-screen flex items-center justify-center">
        <Card className="p-8 max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Wallet no conectada</h2>
      ...
      <div className="container mx-auto px-4 py-16 min-h-screen">
        <Card className="p-8 max-w-2xl mx-auto">
          ...
          <p className="text-gray-400 text-center mb-6">
            {typeof error === 'string' ? error : error?.message || 'No se pudo obtener información del TrustScore'}
          </p>
  ...
  const { score, level, levelName, nextLevelScore, flagged, isStale } = trustScoreInfo;

  // Calcular progreso al siguiente nivel
  const progressToNextLevel = nextLevelScore && nextLevelScore > 0
  ...
  const currentBenefits = LEVEL_BENEFITS[level as keyof typeof LEVEL_BENEFITS] || [];
  const nextLevelBenefits = LEVEL_BENEFITS[(level + 1) as keyof typeof LEVEL_BENEFITS] || [];
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">🏆 TrustScore</h1>
            <p className="text-gray-400">Sistema de reputación del protocolo</p>
          </div>
          <TrustScoreBadge
            score={score}
            level={level}
        ...
        {/* Alertas */}
        {flagged && (
          <Card variant="glass" className="p-6 mb-6 border-2 border-red-500/50 bg-red-500/10">
          <div className="flex items-start gap-4">
            <AlertTriangle className="w-6 h-6 text-red-400 shrink-0 mt-1" />
            <div>
              <h3 className="text-lg font-bold text-red-400 mb-2">⚠️ Cuenta Flagged</h3>
              <p className="text-gray-300 mb-2">
                Tu cuenta ha sido marcada por actividad sospechosa. Algunos beneficios pueden estar limitados.
              </p>
              <p className="text-sm text-gray-400">
                Contacta al equipo de soporte si consideras que esto es un error.
              </p>
            </div>
          </div>
          </Card>
        ...
        {isStale && (
          <Card variant="glass" className="p-4 mb-6 border border-yellow-500/50 bg-yellow-500/10">
          <div className="flex items-center gap-3">
            <AlertTriangle className="w-5 h-5 text-yellow-400" />
            <p className="text-sm text-gray-300">
              Tu TrustScore no se ha actualizado recientemente. Interactúa con el protocolo para mejorarlo.
            </p>
          </div>
          </Card>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="mb-8">
            <div className="flex items-end gap-4 mb-4">
              ...
              <div className="text-3xl font-bold text-gray-400 mb-2">/ {nextLevelScore || '∞'}</div>
            </div>
            ...
            {nextLevelScore && nextLevelScore > 0 && (
              <div>
                <div className="flex justify-between text-sm text-gray-400 mb-2">
                  ...
                  <span>Nivel {level + 1}: {progressToNextLevel.toFixed(1)}%</span>
                </div>
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Necesitas {nextLevelScore - score} puntos más para el siguiente nivel
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="text-center py-4">
              <div className="text-5xl font-bold text-white mb-2">{level}</div>
              <div className="text-lg text-gray-400">{levelName}</div>
              <Badge variant={flagged ? 'danger' : 'success'} className="mt-4">
          ...
          <Card className="p-6">
            <h3 className="text-lg font-bold text-white mb-4">¿Cómo mejorarlo?</h3>
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-2">
                <CheckCircle className="w-4 h-4 text-green-400 shrink-0 mt-0.5" />
                <span className="text-gray-300">Forja geodas regularmente</span>
              </div>
              <div className="flex items-start gap-2">
                <CheckCircle className="w-4 h-4 text-green-400 shrink-0 mt-0.5" />
                <span className="text-gray-300">Mantén miners en staking</span>
              </div>
              <div className="flex items-start gap-2">
                <CheckCircle className="w-4 h-4 text-green-400 shrink-0 mt-0.5" />
                <span className="text-gray-300">Participa en ciclos de minería</span>
              </div>
              <div className="flex items-start gap-2">
                <CheckCircle className="w-4 h-4 text-green-400 shrink-0 mt-0.5" />
                <span className="text-gray-300">Completa verificación PoH</span>
              </div>
            </div>
          </Card>
        ...
        <h2 className="text-xl font-bold text-white mb-4">💡 Sobre TrustScore</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-300">
          <div>
            <h3 className="font-bold text-white mb-2">¿Qué es TrustScore?</h3>
            <p>
              TrustScore es un sistema de reputación on-chain que mide tu participación 
              y confiabilidad en el protocolo ScorchCore. A mayor score, mayores beneficios.
            </p>
          </div>
          <div>
