...
import type { IForgeContract, IRecipeRegistry, ISupplyTracker, ForgeResult, HatchResult, MaterialInput, ForgeEvents, Recipe } from '../interfaces';
import { 
  FORGEFACTORY_ABI, 
  RECIPEREGISTRY_ABI, 
  SUPPLYTRACKER_ABI 
...
class ForgeContract implements IForgeContract {
  ...
  async forgeRecipe(
    ...
    const tx = await this.contract.forgeGeode(category, geodeType, mementosToUse, axieIds);
  ...
  async hatchGeode(geodeId: bigint): Promise<HatchResult> {
    ...
    return {
      success: receipt.status === 1,
      minerId: receipt.events?.[0]?.args?.tokenId,
      power: BigInt(0),
      efficiency: BigInt(0),
      isCritical: false,
      transaction: {
