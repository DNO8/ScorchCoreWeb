...
export function useGeodeStaking() {
  ...
  const stake = useCallback(async (geodeId: bigint) => {
    ...
    if (!contract || !geodeContract) throw new Error('Contract not available');

    setIsLoading(true);
    setError(null);

    try {
      log.info('Staking geode', { geodeId: geodeId.toString(), user: address });
      ...
      const isApprovedForAll = await geodeContract.isApprovedForAll(address, stakingAddress);
      
      // Si no está aprobado, aprobar primero
      if (approved.toLowerCase() !== stakingAddress.toLowerCase() && !isApprovedForAll) {
        log.info('Geode not approved, requesting approval...', { geodeId: geodeId.toString() });
        const approveTx = await geodeContract.approve(stakingAddress, geodeId);
        // Esperar confirmación de la transacción
        const receipt = await approveTx.wait?.() || await approveTx;
        log.info('Geode approved successfully', { geodeId: geodeId.toString(), receipt });
      }
      
      // Ahora stakear (usando interfaz tipada)
      log.info('Executing stake transaction...', { geodeId: geodeId.toString() });
      const tx = await contract.stakeGeode(geodeId);
      const receipt = await tx.wait?.() || await tx;
      log.info('Geode staked successfully', { geodeId: geodeId.toString(), receipt });
      return tx;
    } catch (err) {
   ...
   * Unstakea una Geoda
   */
  const unstake = useCallback(async (geodeId: bigint) => {
    if (!address) throw new Error('Wallet not connected');
    ...
    try {
      log.info('Unstaking geode', { geodeId: geodeId.toString(), user: address });
      const tx = await contract.unstakeGeode(geodeId);
      const receipt = await tx.wait?.() || await tx;
      log.info('Geode unstaked successfully', { geodeId: geodeId.toString(), receipt });
      return tx;
    } catch (err) {
   ...
   */
  const getUserStakingPower = useCallback(async (userAddress?: Address): Promise<bigint> => {
   ...
   */
  const getStakedGeodes = useCallback(async (userAddress?: Address): Promise<bigint[]> => {
    const targetAddress = userAddress || address;
    if (!targetAddress) throw new Error('No address provided');
   ...
   */
  const getStakeInfo = useCallback(async (geodeId: bigint): Promise<GeodeStakeInfo | null> => {
    const contract = getContract();
    if (!contract) throw new Error('Contract not available');
